...
<form method="POST" action="<?= url('projects/' . $project['id'] . '/update') ?>" class="bg-white rounded-xl shadow-sm border border-stone-200 p-6 max-w-2xl space-y-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" step="0.01" min="0" name="agreed_cost" required value="<?= e((string) $project['agreed_cost']) ?>" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm">
        </div>
        <div>
