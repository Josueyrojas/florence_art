...
<form method="POST" action="<?= url('labor/' . $labor['id'] . '/update') ?>" class="bg-white rounded-xl shadow-sm border border-stone-200 p-6 max-w-lg space-y-4">
    ...
    <div>
        ...
        <input type="number" step="0.01" inputmode="decimal" name="hours" value="<?= e((string) $labor['hours']) ?>" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm">
    </div>
    <div>
        <label class="block text-sm font-medium text-stone-600 mb-1">Monto *</label>
        <input type="number" step="0.01" inputmode="decimal" name="amount" required value="<?= e((string) $labor['amount']) ?>" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm">
    </div>
    <div>
